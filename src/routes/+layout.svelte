...
<svelte:head>
  ...
  <meta
    name="description"
    content="Test your football knowledge with animated goal diagrams. Guess the team, year, and scorer — new goals every day."
  />
  <meta property="og:type" content="website" />
  <meta property="og:site_name" content="Top Bins Daily" />
  <meta
    property="og:title"
    content="Top Bins Daily - Test Your Football Knowledge Every Day"
  />
  <meta
  ...
  {#if !dev}
    <script
      ...
      data-website-id="691ee1c5-962d-4809-b7a5-45f12d10a884"
    ></script>
  {/if}
</svelte:head>
...
{#if $navigating}
  <div class="fixed top-0 left-0 right-0 z-[100] h-1 bg-primary/20">
    <div
      class="h-full bg-primary animate-[loading_1s_ease-in-out_infinite]"
    ></div>
  </div>
{/if}
...
<footer class="py-12 px-6 border-t border-border bg-surface">
  <div class="max-w-5xl mx-auto">
    <div class="flex flex-col sm:flex-row items-center justify-between gap-6">
      <div class="flex items-center gap-3">
        ...
        <span class="font-bold text-lg tracking-tight"
          >Top Bins <span class="text-primary">Daily</span></span
        >
      </div>
      ...
      <div class="flex gap-8 text-sm">
        <a
          href="/play"
          class="text-text-muted hover:text-primary transition-colors">Play</a
        >
        {#if showLeaderboard}
          <a
            href="/leaderboard"
        ...
        <a
          href="/editor"
          class="text-text-muted hover:text-primary transition-colors">Create</a
        >
        <a
          href="/profile"
          class="text-text-muted hover:text-primary transition-colors"
    ...
    <div
      class="mt-8 pt-8 border-t border-border text-center text-text-muted text-sm"
    >
